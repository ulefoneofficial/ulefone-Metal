...
{
	//disable_irq_nosync(fingerpirnt_irq);
...
//     fingerprint_irq_registration();
        ...
        if (node){
                ...
                if (int_mode == EDGE_TRIGGER_RISING){
                        ret = request_irq(fingerpirnt_irq,
                                fp_interrupt_edge, IRQF_TRIGGER_RISING,
                                "fingerprint", NULL);
                }
                else if(int_mode == LEVEL_TRIGGER_LOW) {
                        ret = request_irq(fingerpirnt_irq,fp_interrupt_level, IRQ_TYPE_LEVEL_LOW,"fingerprint", NULL);
                }
                if (ret > 0){
