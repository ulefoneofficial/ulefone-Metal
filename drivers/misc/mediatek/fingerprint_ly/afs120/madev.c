...
#define GPIO_FINGER_EN_PIN                  (GPIO127 | 0x80000000)
#ifndef MTK
...
void finger_en_h(void) {
	mt_set_gpio_out(GPIO_FINGER_EN_PIN, 1);
}

void finger_en_l(void) {
	mt_set_gpio_out(GPIO_FINGER_EN_PIN, 0);	
}
...
#define MADEV_INT_PIN   GPIO9
//zhangchongyong add 20151010 begin
#if defined(LYCONFIG_AUTO_PLATFORM_NAME_F5)||defined(LYCONFIG_AUTO_PLATFORM_NAME_F5B)||defined(LYCONFIG_AUTO_PLATFORM_NAME_F2) || defined(LYCONFIG_AUTO_PLATFORM_NAME_F2P)
#define GPIO_IRTX_LDO_EN (GPIO89 | 0x80000000)
#endif
//zhangchongyong add 20151010 end
static void mt_spi_set_mod(void) {
	...
	#endif	
	//��ֹINT©��
	mt_set_gpio_mode(MADEV_INT_PIN,GPIO_MODE_00);  
	mt_set_gpio_dir(MADEV_INT_PIN,GPIO_DIR_OUT);
	mt_set_gpio_pull_select(MADEV_INT_PIN, GPIO_PULL_UP);
	mt_set_gpio_pull_enable(MADEV_INT_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_out(MADEV_INT_PIN,GPIO_OUT_ONE);
	mdelay(10);
	//set dir pull to save power
	//mt_set_gpio_mode(GPIO_SPI_CS_PIN, GPIO_SPI_CS_PIN_M_GPIO);
	
	mt_set_gpio_mode(SPI_CS_PIN, SPI_CS_PIN_MODE);		
...
#endif
//zhangchongyong add 20151010 begin ldo poweron
#if defined(LYCONFIG_AUTO_PLATFORM_NAME_F5)||defined(LYCONFIG_AUTO_PLATFORM_NAME_F5B)||defined(LYCONFIG_AUTO_PLATFORM_NAME_F2) || defined(LYCONFIG_AUTO_PLATFORM_NAME_F2P)
	mt_set_gpio_mode(GPIO_IRTX_LDO_EN, GPIO_MODE_00);                                                                                                                      
       mt_set_gpio_dir(GPIO_IRTX_LDO_EN, GPIO_DIR_OUT);
       mt_set_gpio_out(GPIO_IRTX_LDO_EN, GPIO_OUT_ONE);
#endif
//zhangchongyong add 20151010 end
...
#ifdef MTK
	struct mt_chip_conf *conf = (struct mt_chip_conf *) smas->spi->controller_data;
	conf->com_mod = count>32? DMA_TRANSFER: FIFO_TRANSFER;
	spi_setup(smas->spi);
#endif
...
static long mas_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
	...
	case IOCTL_DEBUG:
		...
		break;
	case IOCTL_SPEED:		
		smas->spi->max_speed_hz = smas->speed = (u32)arg;
		spi_setup(smas->spi);
		break;
...
#endif
//zhangchongyong add 20151010 begin ldo powerdown
#if defined(LYCONFIG_AUTO_PLATFORM_NAME_F5)||defined(LYCONFIG_AUTO_PLATFORM_NAME_F5B)||defined(LYCONFIG_AUTO_PLATFORM_NAME_F2) || defined(LYCONFIG_AUTO_PLATFORM_NAME_F2P)
	mt_set_gpio_mode(GPIO_IRTX_LDO_EN, GPIO_MODE_00);                                                                                                                      
       mt_set_gpio_dir(GPIO_IRTX_LDO_EN, GPIO_DIR_OUT);
       mt_set_gpio_out(GPIO_IRTX_LDO_EN, GPIO_OUT_ZERO);
#endif
//zhangchongyong add 20151010 end
	printd("%s: end\n", __func__);
...
static int mas_probe(struct spi_device *spi) {
	...
	printd("%s: start\n", __func__);
	smas->spi = spi;
	spi->max_speed_hz = smas->speed = 10*000000;
	spi_setup(spi);
