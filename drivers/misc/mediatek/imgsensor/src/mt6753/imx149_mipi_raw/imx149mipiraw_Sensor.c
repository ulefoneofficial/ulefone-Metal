...
static imgsensor_info_struct imgsensor_info = {
    ...
    //F5C
    .checksum_value = 0x4ea69c99,//0x9e08861f,
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("hehe i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                IMX149MIPI_flag = 1;
                return ERROR_NONE;
            }
            LOG_INF("hehe Read sensor id fail, id: 0x%x 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
