...
static kal_uint8 Val28[2][3] =
{
	{0x93,0xa2,0xb8}, //preview
	{0x13,0x22,0x38}, //capture
};


static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC8003_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define ANALOG_GAIN_6 390  // 6.100x
...
{	
	...
	//	LOG_INF("GC8003MIPI analogic gain 1x, GC8003MIPI add pregain = %d\n",temp);
		gainlevel = 0;
	}
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
	//	LOG_INF("GC8003MIPI analogic gain 1.375x , GC8003MIPI add pregain = %d\n",temp);
		gainlevel = 0;
	}
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
	//	LOG_INF("GC8003MIPI analogic gain 1.891x , GC8003MIPI add pregain = %d\n",temp);
		gainlevel = 1;
	}
	else if((ANALOG_GAIN_4<= iReg)&&(iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
	//	LOG_INF("GC8003MIPI analogic gain 2.625x , GC8003MIPI add pregain = %d\n",temp);
		gainlevel = 1;
	}
	else if((ANALOG_GAIN_5<= iReg)&&(iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
	//	LOG_INF("GC8003MIPI analogic gain 3.734x , GC8003MIPI add pregain = %d\n",temp);
		gainlevel = 2;
	}
	else if(ANALOG_GAIN_6<= iReg)
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
	//	LOG_INF("GC8003MIPI analogic gain 6.1x , GC8003MIPI add pregain = %d\n",temp);
		gainlevel = 2;
	}
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x9a,0x79);
	write_cmos_sensor(0x9b,0x7a);
	write_cmos_sensor(0x9c,0x7b);
	write_cmos_sensor(0x9d,0x7c);
	write_cmos_sensor(0x9e,0x1c);
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                printk("<gc8003>i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            //LOG_INF("Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            printk("<gc8003>Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
