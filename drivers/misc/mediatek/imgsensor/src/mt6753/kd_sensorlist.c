...
#ifndef LYCONFIG_EXCHANGE_BACK_FRONT_CAMERA_SUPPORT
		...
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (sensorID == 0) {    /* not implement this feature ID */
					...
					printk(" Not implement!!, use old open function to check\n");
...
bool Get_Cam_Regulator(void)
{
	...
	if (1) {
		...
		if (node) {
			...
			name2 = regulator_get(sensor_device, "vcamd_sub"); /*check customer definition*/
...
bool _hwPowerOn(KD_REGULATOR_TYPE_T type, int powerVolt)
{
	...
	if (!IS_ERR(reg)) {
		if (regulator_set_voltage(reg , powerVolt, powerVolt) != 0) {
			printk("[_hwPowerOn]fail to regulator_set_voltage, powertype:%d powerId:%d\n", type, powerVolt);
			return ret;
	}
...
static int CAMERA_HW_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* AF_VCC */
	//added by xiehaifei 20160623 for imx149 i2c case the switch charge ic i2c error when shutdown charge!	
	#if defined(IMX149_MIPI_RAW_GQ) || defined(IMX149_MIPI_RAW)
	_hwPowerOn(VCAMAF, 2800000);
	#endif
