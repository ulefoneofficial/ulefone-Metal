...
#include "hi841_mipi_raw_Sensor.h"
...
{{ 3264, 2448,   16,  6, 3264, 2448, 3264, 2448, 0000, 0000, 3264, 2448,    2,  2, 3264, 2448}, // Preview
// { 3264, 2448,    0,    0, 3264, 2448,  3264, 2448, 0000, 0000, 1632, 1224,      0,    0, 1632, 1224},
 ...
 { 3264, 2448,     0,    0, 3264, 2448,  3264, 2448, 0000, 0000, 1632, 1224,      0,    0, 2432, 1220}, // video
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	LOG_INF("set_gain");
// AG = 256/(B[7:0] + 32)
	// hi841\BA\C7\C2\E7\BBٻ\FD8\C7\DC\CC\CF?gain,(7 * 255 / 256)\C7ܿ\F4\BB\FAgain
	if(gain > 4080)  
	{
		gain= 4080;
	}
	if(gain < 58)  // gainŪreg\BA\C7\C2\E7?\C0\A7255
	{
		gain = 58;
	}
	...
	else
	{
		...
		write_cmos_sensor(0x0205, 0);   // 8\C7\DC\CC\CF?gain
...
write_cmos_sensor(0x0345, 0x28); // x_addr_start_Lo (40, Display?\BD\D3 20-23 ? Ͼ\C9\DC? 24 \B2\BF\C8\D8 \CCȏ\B0\D1)
write_cmos_sensor(0x0346, 0x00); // y_addr_start_Hi                                               
write_cmos_sensor(0x0347, 0x28); // y_addr_start_Lo (40, Display?\BD\D3 22-23 ? Ͼ\C9\DC? 24 \B2\BF\C8\D8 \CCȏ\B0\D1)
write_cmos_sensor(0x0348, 0x0C); // x_addr_end_Hi                                                 
write_cmos_sensor(0x0349, 0xE7); // x_addr_end_Lo (3303, Display?\BD\D3 3320-3323 ? Ͼʤ)           
write_cmos_sensor(0x034A, 0x09); // y_addr_end_Hi                                                 
write_cmos_sensor(0x034B, 0xB7); // y_addr_end_Lo (2487, Display?\BD\D3 2504-2505 ? Ͼʤ)           
			...
			do {
				*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
				if (*sensor_id == imgsensor_info.sensor_id) {				
					printk("Hi841 i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
					return ERROR_NONE;
				}	
				printk("Hi841 Read sensor id fail, i2c write id=0x%x,id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
