...
s32 gesture_event_handler(struct input_dev * dev)
{
   ...
   /*     if (likely(need_chk)) { 
	...
	//	key_code = KEY_POWER;//custom
	//	input_report_key(dev, key_code, 1);
	//	input_sync(dev);
	//	input_report_key(dev, key_code, 0);
	//	input_sync(dev);

    GTP_DEBUG("WENG 0x%x = 0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE,
        doze_buf[0], doze_buf[1], doze_buf[2], doze_buf[3]);
                    ...
                    )
                {
                    if (doze_buf[0] != 0x5E)
                    {
                        GTP_DEBUG("Wakeup by gesture(%c), light up the screen!", doze_buf[2]);
                    }
                    else
                    {
                        GTP_DEBUG("Wakeup by gesture(^), light up the screen!");
                    }
		     					 switch(doze_buf[0])
		     					 {
			  							case 'o':		type = 4;
                    ...
                    (doze_buf[0] == 0xAB) || (doze_buf[0] == 0xBA) )
                {
                    ...
                    GTP_DEBUG("%s slide to light up the screen!", direction[type]);
                    //doze_status = DOZE_WAKEUP;
                    input_report_key(dev, gesture_key_array[type], 1);
                    input_sync(dev);
                    input_report_key(dev, gesture_key_array[type], 0);
...
;
                ...
                {
                    GTP_DEBUG("Double click to light up the screen!");
		      					//doze_status = DOZE_WAKEUP;
