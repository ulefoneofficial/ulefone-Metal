...
static int of_get_ft5x0x_platform_data(struct device *dev)
{
	...
	if (dev->of_node) {
		...
		match = of_match_device(of_match_ptr(ft5x0x_of_match), dev);
		if (!match) {
...
static int touch_event_handler(void *unused)
{
...
#ifdef TPD_HAVE_BUTTON
	if (1/*tpd_dts_data.use_tpd_button*/) {
	...
	do {
		...
		TPD_DMESG("touch_event_handler start\n");
	...
	TPD_DMESG("touch_event_handler exit\n");
