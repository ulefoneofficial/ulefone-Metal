...
int fts_Gesture_init(struct input_dev *input_dev)
{
	...
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_P);
...
static void fts_check_gesture(struct input_dev *input_dev,int gesture_id)
{
	...
	switch(gesture_id)
	{
	        ...
	        case GESTURE_DOUBLECLICK:
	                input_report_key(input_dev, KEY_GESTURE_P, 1);
	                input_sync(input_dev);
	                input_report_key(input_dev, KEY_GESTURE_P, 0);
	                input_sync(input_dev);
	                break;
	        case GESTURE_O:
